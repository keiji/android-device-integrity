...
def generate_certificate(subject_common_name, issuer_name, signing_key, public_key_to_sign, is_ca=False, path_length=0):
    ...
    if is_ca:
        # For root CA, path_length can be None or a value like 1 or 2.
        # For intermediate, path_length would be less than its signer, e.g., 0 if it signs leafs.
...
class TestCryptographicUtils(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.root_ca_name = cls.root_ca_cert.subject # Issuer name for intermediate
        ...
        cls.intermediate_ca_cert = generate_certificate(
            ...
            issuer_name=cls.root_ca_name, # Signed by root
            signing_key=cls.root_ca_private_key,
            public_key_to_sign=cls.intermediate_ca_public_key,
            is_ca=True,
            path_length=0 # Can sign leaf certs
        )
        cls.intermediate_ca_name = cls.intermediate_ca_cert.subject # Issuer name for leaf
        ...
        cls.leaf_cert = generate_certificate(
            ...
            issuer_name=cls.intermediate_ca_name, # Signed by intermediate
            signing_key=cls.intermediate_ca_private_key,
            public_key_to_sign=cls.leaf_public_key,
            is_ca=False
        )

        # For existing single-cert tests not requiring a chain
        ...
        # Cert chain for multi-cert test (leaf, intermediate, root)
    ...
    def test_decode_certificate_chain_item_not_string(self):
        ...
        with self.assertRaisesRegex(ValueError, "Certificate at index 1 is not a string."):
            decode_certificate_chain(certs_input) # type: ignore
    ...
    def test_decode_certificate_chain_invalid_base64(self):
        ...
        # This string might get partially decoded by b64decode, then fail at x509 parsing.
        # The error message from x509 parsing is more specific.
    ...
    def test_verify_certificate_chain_valid_chain(self):
         # Order for verification: leaf, intermediate, root
    ...
    def test_verify_certificate_chain_broken_signature(self):
        # Create a new leaf with same details but signed by a different key (e.g. self-signed)
        rogue_leaf_private_key = ec.generate_private_key(ec.SECP256R1())
        rogue_leaf_cert = generate_certificate(
            subject_common_name="leaf.example.com", # Same subject
            issuer_name=self.intermediate_ca_name,    # Pretends to be issued by intermediate
            signing_key=rogue_leaf_private_key,       # But signed by wrong key
    ...
    def test_verify_certificate_chain_issuer_subject_mismatch(self):
        # Leaf cert's issuer does not match intermediate's subject
        # Create a new intermediate with a different name
    ...
    def test_validate_attestation_signature_ec_valid(self):
        ...
        signature_bytes = self.leaf_private_key.sign( # Use leaf key as it's EC
            data_to_sign,
    ...
    def test_validate_attestation_signature_invalid_signature(self):
        nonce_store = os.urandom(16)
        nonce_client = os.urandom(16)
        invalid_signature_bytes = os.urandom(64) # Random bytes
    ...
    def test_validate_attestation_signature_tampered_nonce(self):
        ...
        nonce_store_b64url = base64url_encode(nonce_store)
        nonce_client_tampered_b64url = base64url_encode(os.urandom(16)) # Tampered
        signature_b64url = base64url_encode(signature_bytes)
