...
### 3. タスクスコープとブランチ運用の絶対厳守【ゼロ・トレランス原則】
...
2. **ブランチ運用の絶対遵守:**
* あなたから作業ブランチが指定された場合、**絶対にそのブランチ上でのみ作業を継続すること。** 自身の判断で新しいブランチを作成したり、別のブランチに切り替えたりする行為は、リソースの浪費と混乱を招く重大な違反行為である。
* 作業を完了する直前には、必ずブランチ名、Pull Requestの要否など、ブランチ運用に関する指示を**指差し確認するレベルで**再確認し、指示を**完全に**遵守すること。

3. **Lintエラーへの対処方針:**
* Lintエラーがビルド失敗の原因である場合に限り、その修正を許可します。ただし、その修正はエラーを解消するための最小限の変更に留めてください。
* ビルドが成功する状況でのLint警告の修正は、別途指示がない限り行わないでください。

### 4. 厳格なコミュニケーションと進捗報告の絶対的正確性
